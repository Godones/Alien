...
impl <T> IntrLock<T> {
    ...
    pub fn lock(&self) -> IntrLockGuard<T> {
        ...
        if self.cnt.load(core::sync::atomic::Ordering::Relaxed) == 0 {
            self.hart_id.swap(arch::hart_id(), core::sync::atomic::Ordering::Relaxed);
...
impl <T> Drop for IntrLockGuard<'_,T> {
    fn drop(&mut self) {
        ...
        if arch::hart_id() != self.lock.hart_id.load(core::sync::atomic::Ordering::Relaxed) {
        ...
        self.data = None;
        if val == 0 && self.lock.is_enabled_before.load(core::sync::atomic::Ordering::Relaxed) {
