...
use domain_helper::{DomainSyscall, SharedHeapAllocator, TaskShimImpl};
...
impl DomainLoader {
    ...
    pub fn call<T: ?Sized>(&self, id: u64) -> Arc<T> {
        type F<T> = fn(
            Box<dyn libsyscall::Syscall>,
            u64,
            Box<dyn rref::SharedHeap>,
            Box<dyn libsyscall::KTaskShim>,
        ) -> Arc<T>;
        ...
        let task_shim = Box::new(TaskShimImpl);

        let syscall_ptr = Box::into_raw(syscall);
        let heap_ptr = Box::into_raw(heap);
        let task_shim_ptr = Box::into_raw(task_shim);
        ...
        domain_helper::register_domain_taskshim_resource(id, task_shim_ptr as usize);
        unsafe {
            main(
                Box::from_raw(syscall_ptr),
                id,
                Box::from_raw(heap_ptr),
                Box::from_raw(task_shim_ptr),
            )
        }
    }
    ...
    fn load_program(&mut self, elf: &ElfFile) -> AlienResult<()> {
        elf.program_iter()
            ...
            .for_each(|ph| {
                ...
                // save region
                let meta = RegionMeta {
                    vm: VmArea::new(vaddr..end_vaddr, permission),
    ...
    fn reload_program(&self, elf: &ElfFile) -> AlienResult<()> {
        for meta in &self.regions {
            ...
            if meta.vm.permission().contains(MappingFlags::W) {
                info!(
