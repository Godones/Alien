...
pub use rtc::{RTC_DEVICE,RTCDevice};
...
pub fn init_device(task_func:Box<dyn DeviceWithTask>) {
    ...
    match dtb.probe_rtc() {
        ...
        None => { println!("There is no rtc device"); },
    }

    match dtb.probe_uart() {
        Some(uart) => init_uart(uart),
        None => { println!("There is no uart device"); },
    }
...
fn init_block_device(blk: prob::DeviceInfo, mmio_transport: Option<MmioTransport>) {
    use drivers::block_device::{GenericBlockDevice, VirtIOBlkWrapper};
...
fn init_net(nic: prob::DeviceInfo) {
