...
use interface::{Basic, DevFsDomain, DirEntryWrapper, FsDomain, InodeID};
use log::info;
...
#[derive(Debug)]
pub struct DevFsDomainProxy {
    id: u64,
    domain: Box<dyn DevFsDomain>,
}

impl DevFsDomainProxy {
    pub fn new(id: u64, domain: Box<dyn DevFsDomain>) -> Self {
        Self { id, domain }
    }
}
...
impl FsDomain for DevFsDomainProxy {
    fn init(&self) -> AlienResult<()> {
        self.domain.init()
    }
...
impl DevFsDomain for DevFsDomainProxy {
    fn register(&self, rdev: u64, device_domain_name: &RRefVec<u8>) -> AlienResult<()> {
        if self.is_active() {
            self.domain.register(rdev, device_domain_name)
        } else {
            Err(constants::AlienError::DOMAINCRASH)
        }
    }
