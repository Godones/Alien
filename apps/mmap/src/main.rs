...
pub fn main() {
    ...
    let start = mmap(0, stat.st_size as usize, ProtFlags::PROT_WRITE | ProtFlags::PROT_READ, MapFlags::MAP_SHARED, fd as usize, 0);
    assert!(start > 0);
    ...
    let mmap = unsafe {
        core::slice::from_raw_parts(start as *const u8, stat.st_size as usize)
    };
    println!("The content of the file is {}", core::str::from_utf8(mmap).unwrap());
