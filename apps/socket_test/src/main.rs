...
fn tcp_test() {
    ...
    if pid !=0 {
        ...
        assert!(bind(sockfd as usize, &addr as *const Sockaddr, core::mem::size_of::<Sockaddr>()) >= 0);
        ...
        loop {
            let connfd: isize = accept(sockfd as usize, &mut peeraddr as *mut Sockaddr, &mut peerlen as *mut usize);
            assert!(connfd >= 0);
            let mut buf = [0u8; BUF_SIZE];
            let recvnum = recv(connfd as usize, &mut buf as *mut u8, BUF_SIZE, 0);
            assert!(recvnum >= 0);
            println!("tcp server receive a message: {}",str::from_utf8(&buf).unwrap());
        ...
        assert!(connect(sockfd as usize, &addr as *const Sockaddr, core::mem::size_of::<Sockaddr>()) >= 0);
        send(sockfd as usize, buf.as_ptr() as *const u8, buf.len(), 0);
        let recvnum = recv(sockfd as usize, buf.as_ptr() as *mut u8, BUF_SIZE, 0);
        assert!(recvnum >= 0);
        println!("tcp client receive a message: {}",str::from_utf8(&buf).unwrap());
...
fn udp_test() {
    ...
    if pid !=0 {
        ...
        loop {
            ...
            let mut peerlen: usize = 0;
            let mut peeraddr: Sockaddr = addr.clone();
            let recvnum = recvfrom(sockfd as usize, &mut buf as *mut u8, BUF_SIZE, 0, &mut peeraddr as *mut Sockaddr, &mut peerlen as *mut usize);
            assert!(recvnum >= 0);
            println!("udp server receive a message: {}",str::from_utf8(&buf).unwrap());
            break;
        }
        close(sockfd as usize);
        ...
        let addr = Sockaddr::new(Domain::AF_INET, INTADDR, PORT);
        let buf: &[u8] = b"Hello, this is Alien's client";
        sendto(sockfd as usize, buf.as_ptr() as *const u8, buf.len(), 0, &addr as *const Sockaddr, core::mem::size_of::<Sockaddr>());
