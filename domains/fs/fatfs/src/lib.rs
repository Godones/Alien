...
use interface::{
    Basic, DirEntryWrapper, DomainType, FsDomain, InodeID, VfsDomain,
};
use ksync::Mutex;
...
use crate::shim::MountPointShimInode;
...
type DiskFs = FatFs<ProviderImpl, Mutex<()>>;
...
impl FsDomain for FatFsDomain {
    ...
    fn mount(&self, mount_point: &RRefVec<u8>, dev_inode: Option<InodeID>) -> AlienResult<InodeID> {
        let mount_point = core::str::from_utf8(mount_point.as_slice())
            .unwrap()
            .to_string();
        let shim_inode = MountPointShimInode::new(dev_inode.ok_or(AlienError::ENODEV)?);
        let root = self.fs.i_mount(0, "/", Some(Arc::new(shim_inode)), &[])?;
        let inode_id = self
            .inode_index
        ...
        vfscore::path::print_fs_tree(&mut FakeOut, root.clone(), mount_point, true).unwrap();
